...
int create_messages() {
	...
	if (RegSetValueEx(key, _T("EventMessageFile"), 0, REG_SZ, (const unsigned char*)path, (unsigned long)(_tcslen(path) + 1) * sizeof(TCHAR)) != ERROR_SUCCESS)
	{
		return 3;
	}
	unsigned long types = EVENTLOG_INFORMATION_TYPE | EVENTLOG_WARNING_TYPE | EVENTLOG_ERROR_TYPE;
	if (RegSetValueEx(key, _T("TypesSupported"), 0, REG_DWORD, (const unsigned char*)& types, sizeof(types)) != ERROR_SUCCESS)
	{
		return 4;
	}
